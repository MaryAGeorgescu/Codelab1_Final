...
public class Story : MonoBehaviour //Gamemanger for a simple Zork type game
{
  JSONArray jsonArray; //a data structure that holds data from a json file
    ...
    StoryNode currentNode; //current zork location
    ...
    void Start()
    {
        ...
        string jsonString = File.ReadAllText(Application.dataPath + "/StoryNav.json"); //get all the text out of json file
    ...
    void SetCurrentNode(int locationNum) //get the current node based on locationNum
    {
        ...
        currentNode = new StoryNode(); //make a new ZorkNode
    ...
    void UpdateUI(StoryNode currentNode) //update the UI based on currentNode
    {
